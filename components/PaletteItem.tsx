...
const PaletteItem = ({ rgb, hex }: PaletteItemProps) => {
    ...
    return (
        ...
        >
            <span
                className="flex items-center font-semibold text-xs bg-[rgba(255,255,255,0.14)] justify-between w-full cursor-pointer px-2 py-2 absolute bottom-0 border-t-2 border-t-border rounded-b-lg"
                onClick={(e) => {
                    copyToClipboard(e);
                    setCopied(true);
                    setTimeout(() => {
            ...
            >
                {copied ? "Copied!" : colorFormat ? rgb : hex}
